<%
  ...
  from {{ cookiecutter.python_package }} import __version__ as package_version
...
<%def name="show_module(module)">
  ...
  <h1 class="title">${'Namespace' if module.is_namespace else  \
                      ...
                      'Module'} <code>${module.name}</code><small><sub>v${package_version}</sub></small></h1>
  </header>
...
<footer id="footer" style="position:fixed; bottom: 0px; width: 100%; text-align: right;background-color:white">
